...
function createAutomationCard(a, i) {
  const shots = a.images.map(img => `<img src="${img}" alt="${a.title} workflow screenshot" loading="lazy" onerror="this.style.display='none'"/>`).join('');
  return `
    <div class="auto-card">
      <div class="auto-shots">${shots}</div>
      <div class="auto-body">
        <h3 class="auto-title">${a.title}</h3>
        <p class="auto-desc">${a.description}</p>
        <div class="project-tags">${a.tags.map(t => `<span class="project-tag">${t}</span>`).join('')}</div>
        <button class="auto-toggle" data-i="${i}">
          <span class="auto-toggle-label">Read case study</span>
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
        </button>
        <p class="auto-case" id="case-${i}">${a.case_study}</p>
        <div class="project-links">
          <a href="${a.github}" target="_blank" rel="noopener" class="project-link">
...
function renderAutomations(list) {
  ...
  document.querySelectorAll('.auto-toggle').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      btn.querySelector('.auto-toggle-label').textContent = isOpen ? 'Hide case study' : 'Read case study';
    });
  });
}
